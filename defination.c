...
// of amount of transfer Data
struct money
{
	char username_to[50];
	char user_person_from[50];
	long int money1;
};
...
int main()
{
	...
	printf("DEVELOPER-Nighaban Hasan");
...
void account(void)
{
	...
	printf("\n\nFATHER's NAME..	");
	scanf("%s", &u1.fath_name);

	printf("\n\nMOTHER's NAME..	");
	scanf("%s", &u1.moth_name);

	printf("\n\nADDRESS..	");
	scanf("%s", &u1.address);

	printf("\n\nACCOUNT TYPE	");
	scanf("%s", &u1.type_of_account);
...
void transfermoney(void)
{
...

	{
		...
		{
			strcpy(m1.username_to, u1.username);
			strcpy(m1.user_person_from, usernamet);
		}
	}
	gotoxy(33, 16);
...
void checkbalance(char username2[])
{
	...
	while (fread(&m1, sizeof(m1), 2, fm))
	{
		if (strcmp(username2, m1.username_to) == 0)
		{
			gotoxy(k, ++l);
			printf("%d", i);
			i++;
			gotoxy(m, ++n);
			printf("%s", m1.user_person_from);
